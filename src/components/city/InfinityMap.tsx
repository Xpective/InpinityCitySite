...
// ============================================================
// PUNKT 5: Status-Farben und Muster für verschiedene Plot-Status
// ============================================================
const STATUS_STYLES = {
  reserved: {
    baseColor: "#ffaa33",
...
// ============================================================
// PUNKT 4: Verbesserte Heatmap mit echten Daten
// ============================================================
function getEnhancedHeatColor(
  plot: InfinityPlot & { 
  ...
  if (plot.historicScore !== undefined) {
    const intensity = Math.min(plot.historicScore / 1000, 1);
    if (plot.side === "left") {
  ...
  if (plot.aetherUses !== undefined) {
    const intensity = Math.min(plot.aetherUses / 10, 1);
...
// ============================================================
// PUNKT 5: Status-basierte Fill-Funktion mit Mustern
// ============================================================
function getStatusBasedFill(plot: InfinityPlot, heatmapMode: boolean): { fill: string; pattern?: string } {
  ...
  const statusStyle = STATUS_STYLES[plot.status as keyof typeof STATUS_STYLES] || STATUS_STYLES.free;
...
export default function InfinityMap({
  ...
  // ============================================================
  // PUNKT 7: State für Plot-Typ-Filter
  // ============================================================
  ...
  // ============================================================
  // PUNKT 11: Memoization für Stats
  // ============================================================
  const plotStats = useMemo(() => {
    return {
      total: filteredPlots.length,
      personal: filteredPlots.filter(p => p.plotKind === "personal-5x5").length,
      community: filteredPlots.filter(p => p.plotKind === "community-25x25").length,
      borderline: filteredPlots.filter(p => p.plotKind === "borderline-25x25").length,
      nexus: filteredPlots.filter(p => p.plotKind === "nexus").length,
    };
  }, [filteredPlots]);

  // ============================================================
  // PUNKT 6: Hover-Handler für Tooltips
  // ============================================================
  ...
  // ============================================================
  // PUNKT 9: Zoom-Handler für Community-Plots
  // ============================================================
  const handlePlotClick = useCallback((plot: InfinityPlot) => {
  ...
  return (
    <div className="mapWrap" id="city-map-capture" ref={containerRef}>
      {/* ============================================================ */}
      {/* PUNKT 7: Filter-Leiste für Plot-Typen */}
      {/* ============================================================ */}
      <div className="filterBar" style={{ marginBottom: "10px", display: "flex", gap: "15px", flexWrap: "wrap" }}>
        <label>
          ...
          /> Persönliche Plots ({plotStats.personal})
        </label>
        <label>
          <input 
            type="checkbox" 
      ...
      <div className="mapLegend">
        ...
        <div><span style={{ color: "#ffaa33", background: "repeating-linear-gradient(45deg, transparent, transparent 5px, rgba(255,255,255,0.2) 5px, rgba(255,255,255,0.2) 10px)" }}>■</span> Reserviert</div>
        <div><span style={{ color: "#44ff44" }}>■</span> Besitzt</div>
        <div><span style={{ color: "#888888", background: "repeating-linear-gradient(45deg, #888, #888 5px, #666 5px, #666 10px)" }}>■</span> Gesperrt</div>
      </div>

      {/* ============================================================ */}
      {/* PUNKT 5: SVG-Defs für Muster */}
      {/* ============================================================ */}
      <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} className="citySvg">
        ...
        <text x={CENTER_X - 520} y={CENTER_Y + 5} fill="rgba(255,235,190,0.95)" fontSize="26" fontWeight="700">INPINITY</text>
        <text x={CENTER_X + 330} y={CENTER_Y + 5} fill="rgba(225,235,255,0.95)" fontSize="26" fontWeight="700">INPHINITY</text>
      </svg>
        ...
        <Tooltip
          title={
            <div style={{ padding: "8px" }}>
              ...
              Status: {STATUS_STYLES[hoveredPlot.status as keyof typeof STATUS_STYLES]?.label || hoveredPlot.status}<br />
              Fraktion: {hoveredPlot.faction}<br />
              {hoveredPlot.owner && `Besitzer: ${hoveredPlot.owner.substring(0, 6)}...${hoveredPlot.owner.substring(38)}`}<br />
              Preis: {hoveredPlot.priceEstimate} PIT<br />
              {hoveredPlot.lastTransferDaysAgo !== undefined && `Letzter Transfer: vor ${hoveredPlot.lastTransferDaysAgo} Tagen`}
            </div>
          }
          open={true}
          placement="top"
      ...
      {/* ============================================================ */}
      {/* PUNKT 9: Zoom-Modal für Community-Plots */}
      {/* ============================================================ */}
      {zoomedPlot && (
        <div
          style={{
        ...
        >
          ...
          >
            <h3 style={{ marginTop: 0 }}>{zoomedPlot.label} - Vergrößerte Ansicht</h3>
            <svg width="400" height="400" viewBox="0 0 400 400">
              ...
              <text x="200" y="230" textAnchor="middle" fill="white" fontSize="12">25x25 Community Plot</text>
            </svg>
            <button
              onClick={() => setZoomedPlot(null)}
              style={{
