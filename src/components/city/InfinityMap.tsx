...
function getStatusBasedFill(
  plot: InfinityPlot,
  heatmapMode: boolean
): { fill: string; pattern?: string } {
  if (heatmapMode) {
  ...
  if (plot.faction === "community") {
    return {
      fill: STATUS_STYLES.community.baseColor,
      pattern: STATUS_STYLES.community.pattern,
    };
  }

  if (plot.faction === "borderline") {
    return {
      fill: STATUS_STYLES.borderline.baseColor,
      pattern: STATUS_STYLES.borderline.pattern,
    };
  }
...
function getOwnerShort(owner?: string | null): string {
  if (!owner) return "—";
...
export default function InfinityMap({
  ...
  const changeZoom = useCallback((delta: number) => {
    setMapZoom((prev) => clamp(Number((prev + delta).toFixed(2)), 0.7, 2.4));
  }, []);
  ...
  return (
    <div className="mapWrap" id="city-map-capture" ref={containerRef}>
        ...
        <div className="mapFilterBar">
          <label>
            <input
              ...
              onChange={(e) =>
                setPlotTypeFilter((prev) => ({
                  ...prev,
                  personal: e.target.checked,
                }))
              }
            />{" "}
            Personal ({plotStats.personal})
          </label>
          ...
          <label>
            ...
            />{" "}
            Borderline ({plotStats.borderline})
          </label>
      ...
      <div
        style={{
          width: `${mapZoom * 100}%`,
          minWidth: mapZoom > 1 ? `${Math.round(SVG_WIDTH * mapZoom)}px` : `${SVG_WIDTH}px`,
      ...
      >
        <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} className="citySvg">
          <defs>
            ...
            <pattern id="solidFill" patternUnits="userSpaceOnUse" width="8" height="8">
        ...
        <Tooltip
          title={
            <div style={{ padding: "8px" }}>
              <strong>{hoveredPlot.label}</strong>
              <br />
              Status: {STATUS_STYLES[hoveredPlot.status]?.label || hoveredPlot.status}
              <br />
              Faction: {hoveredPlot.faction}
              <br />
              Tier: {hoveredPlot.tier}
              <br />
              Owner: {getOwnerShort(hoveredPlot.owner)}
              <br />
              Value: {hoveredPlot.valueModel?.finalEstimate || hoveredPlot.priceEstimate} PIT
              <br />
              {hoveredPlot.provenance?.legacyScore != null && (
                <>
                  Legacy: {hoveredPlot.provenance.legacyScore}
                  <br />
                </>
              )}
              ...
              {hoveredPlot.lastTransferDaysAgo !== undefined &&
                `Last Transfer: ${hoveredPlot.lastTransferDaysAgo} days ago`}
            </div>
          }
          open={true}
          placement="top"
